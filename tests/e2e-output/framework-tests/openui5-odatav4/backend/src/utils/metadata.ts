 ...
 */
export async function buildEntityType(
  db: Knex,
  tableName: string
): Promise<EntityType> {
...
export async function buildMetadata(db: Knex): Promise<string> {
  ...
  const systemEntities = [
    'sys_table', 'sys_column', 'sys_window', 'sys_tab', 'sys_field',
    'sys_reference', 'sys_val_rule', 'sys_user', 'sys_role', 'sys_access'
  ];
...
function generateEdmx(entityTypes: EntityType[], entitySets: EntitySet[]): string {
  ...
  const entityTypesXml = entityTypes.map(et => {
    const keysXml = et.keys.map(k => `        <PropertyRef Name="${k}" />`).join('\n');
    const propsXml = et.properties.map(p => {
...
${propsXml}
  ...
  const entitySetsXml = entitySets.map(es =>
    `      <EntitySet Name="${es.name}" EntityType="${es.entityType}" />`
  ).join('\n');
