...
test.describe('ERDwithAI Generator', () => {
  ...
  test('should display home page with project list', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("New Project")').or(page.locator('button:has-text("Create")')).or(page.locator('[data-testid="new-project-btn"]')).first()).toBeVisible();
  });

  test('should create a new Next.js NestJS project', async ({ page }) => {
    // Click new project button
    await page.click('button:has-text("New Project"), button:has-text("Create"), [data-testid="new-project-btn"]');
    ...
    await page.fill('textarea[name="description"], textarea#project-description', TEST_PROJECTS.nextjsNestjs.description);

    // Submit the form
    await page.click('button[type="submit"]:has-text("Save"), button:has-text("Continue"), button:has-text("Next")');
  ...
  test('should create a new OData UI5 project', async ({ page }) => {
    ...
    await page.fill('textarea[name="description"], textarea#project-description', TEST_PROJECTS.odataUi5.description);
  ...
  test('should design ERD for a project', async ({ page }) => {
    ...
    // Enter ERD code
    const erdEditor = page.locator('textarea[placeholder*="Mermaid"], textarea[name="erdCode"], [data-testid="erd-editor"], .monaco-editor textarea').first();
  ...
  test('should generate Next.js NestJS application', async ({ page }) => {
    ...
    await page.click('button:has-text("Generate"), button:has-text("Build"), button:has-text("Create Application")');

    // Wait for generation to complete
    await expect(page.locator('text=Generating, text=Building, text=Creating').or(page.locator('.progress, [role="progressbar"]'))).toBeVisible();

    // Wait for success message (may take several minutes)
    await expect(page.locator('text=Success, text=Complete, text=Generated, text=Ready').or(page.locator('button:has-text("Deploy"), button:has-text("Run")'))).toBeVisible({ timeout: 300000 });
  });
...
test.describe('Generated Next.js Application', () => {
  ...
  test('should create a new customer', async ({ page }) => {
    ...
    await expect(page.locator('text=success, text=created, text=Success').or(page.locator('h1:has-text("Test Customer")'))).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should view customer details', async ({ page }) => {
    ...
    // Verify detail page
    await expect(page.locator('h1, h2').filter({ hasText: /Customer/i })).toBeVisible();
    await expect(page.locator('text=Test Customer').or(page.locator('text=test@example.com'))).toBeVisible();
  });
  ...
  test('should update a customer', async ({ page }) => {
    ...
    // Verify success
    await expect(page.locator('text=updated, text=success, text=Success')).toBeVisible({ timeout: 5000 });
  });
...
test.describe('Generated OpenUI5 Application', () => {
  ...
  test('should create new item', async ({ page }) => {
    ...
    // Click create button
    await page.click('button:has-text("Create"), button[data-icon="add"], .sapMBtnIcon[data-icon="add"]');
...
test.describe('Complete Workflow Integration', () => {
  test('should complete full workflow: Create -> Design -> Generate -> Deploy', async ({ page }) => {
    // Create project
    await page.goto('/');
    ...
    // Wait for generation
    await expect(page.locator('text=Success, text=Complete, text=Generated')).toBeVisible({ timeout: 300000 });
    ...
    // Wait for deployment
    await expect(page.locator('text=Running, text=Active, text=Deployed')).toBeVisible({ timeout: 60000 });
