...
const FRONTEND_URL = process.env.HOSPITAL_FRONTEND_URL || process.env.FRONTEND_URL || 'http://localhost:3000';
const BACKEND_URL = process.env.HOSPITAL_BACKEND_URL || process.env.BACKEND_URL || 'http://localhost:3001';
...
test.describe('Patient Management', () => {
  ...
  test('should navigate to create patient page', async ({ page }) => {
    ...
    const createBtn = page.locator('button:has-text("Create New"), button:has-text("Create"), button:has-text("New Patient")').first();
  ...
  test('should read a patient record', async ({ request }) => {
    // Create then read
    const created = await (await request.post(`${API_BASE}/patients`, {
  ...
  test('should display patient detail page', async ({ page }) => {
    ...
    const created = await (await page.request.post(`${API_BASE}/patients`, {
...
test.describe('Staff Management', () => {
  ...
  test('should navigate to create staff page', async ({ page }) => {
    ...
    const createBtn = page.locator('button:has-text("Create New"), button:has-text("Create"), button:has-text("Add")').first();
    await createBtn.click();
    await waitForPageReady(page);
...
test.describe('Appointment Management', () => {
  ...
  test('should create an appointment via API', async ({ request }) => {
    ...
    const patient = await (await request.post(`${API_BASE}/patients`, {
...
test.describe('Patient Form - UI Interactions', () => {
  ...
  test('should have cancel/back button on create page', async ({ page }) => {
    ...
    const cancelBtn = page.locator('button:has-text("Cancel"), button:has-text("Back"), a:has-text("Back")').first();
