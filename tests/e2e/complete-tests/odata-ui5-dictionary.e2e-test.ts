...
test.describe('OData $metadata - Dictionary Entity Types', () => {
  ...
  test('$metadata should declare navigation properties for sys_ relationships', async ({ request }) => {
    const response = await request.get(`${ODATA_ROOT}/$metadata`);
  ...
  test('should support $orderby on reference id', async ({ request }) => {
    ...
    if (data.value.length > 1) {
      for (let i = 1; i < data.value.length; i++) {
        expect(data.value[i].sys_reference_id).toBeGreaterThanOrEqual(data.value[i - 1].sys_reference_id);
      }
    }
  });
...
test.describe('OData API - SysColumns Collection', () => {
  ...
  test('should filter mandatory columns', async ({ request }) => {
    const response = await request.get(
      `${ODATA_ROOT}/SysColumns?$filter=is_mandatory eq true`
    );
  ...
  test('should order columns by seq_no', async ({ request }) => {
    const response = await request.get(
      `${ODATA_ROOT}/SysColumns?$orderby=seq_no asc&$top=20`
    );
  ...
  test('should $expand to include sys_table relationship', async ({ request }) => {
    const response = await request.get(
      `${ODATA_ROOT}/SysColumns?$expand=SysTable&$top=5`
    );
...
test.describe('OData API - SysWindows Collection', () => {
  ...
  test('should filter Maintain-type windows', async ({ request }) => {
    const response = await request.get(
      `${ODATA_ROOT}/SysWindows?$filter=window_type eq 'M'`
    );
...
test.describe('OData API - SysTabs Collection', () => {
  ...
  test('should have master tabs at level 0', async ({ request }) => {
    const response = await request.get(
      `${ODATA_ROOT}/SysTabs?$filter=tab_level eq 0`
    );
  ...
  test('should $expand to include SysWindow and SysTable', async ({ request }) => {
    const response = await request.get(
      `${ODATA_ROOT}/SysTabs?$expand=SysWindow,SysTable&$top=3`
    );
...
test.describe('OData API - SysFields Collection (Layout & seq_no)', () => {
  ...
  test('should filter displayed fields', async ({ request }) => {
    const response = await request.get(
      `${ODATA_ROOT}/SysFields?$filter=is_displayed eq true`
    );
  ...
  test('should filter grid-displayed fields', async ({ request }) => {
    const response = await request.get(
      `${ODATA_ROOT}/SysFields?$filter=is_displayed_grid eq true`
    );
  ...
  test('PATCH /odata/SysFields(:id) - should update seq_no for runtime reordering', async ({ request }) => {
    ...
    if (data.value && data.value.length >= 2) {
      ...
      const update1 = await request.patch(
        `${ODATA_ROOT}/SysFields(${field1.sys_field_id})`,
        {
      ...
      const update2 = await request.patch(
        `${ODATA_ROOT}/SysFields(${field2.sys_field_id})`,
        {
  ...
  test('PATCH /odata/SysFields(:id) - should toggle is_displayed', async ({ request }) => {
    ...
    if (data.value && data.value.length > 0) {
      ...
      const hideResponse = await request.patch(
        `${ODATA_ROOT}/SysFields(${field.sys_field_id})`,
        {
...
test.describe('OpenUI5 Frontend - Dictionary-Driven Views', () => {
  ...
  test('should render entity list with columns from sys_field.is_displayed_grid', async ({ page }) => {
    ...
    if (await entityItem.isVisible()) {
      ...
      if (await recordItem.isVisible()) {
        ...
        if (sectionCount > 0) {
          ...
          const sectionTitles = page.locator('.sapUxAPObjectPageSectionTitle, .sapMPanelExpandableTitle');
...
test.describe('OData Performance - Dictionary Queries', () => {
  ...
  test('SysFields query with filter and orderby should respond within 3 seconds', async ({ request }) => {
    const startTime = Date.now();
