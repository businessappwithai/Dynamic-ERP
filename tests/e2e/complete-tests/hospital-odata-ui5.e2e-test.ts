...
const ODATA_BACKEND_URL = process.env.HOSPITAL_ODATA_URL || process.env.ODATA_BACKEND_URL || 'http://localhost:3000';
const UI5_FRONTEND_URL = process.env.HOSPITAL_UI5_URL || process.env.UI5_FRONTEND_URL || 'http://localhost:3004';
...
test.describe('OData - Prescription Entity', () => {
  test('GET /odata/PrescriptionSet - should return prescriptions collection', async ({ request }) => {
...
test.describe('OData - Insurance Provider Entity', () => {
  test('GET /odata/InsuranceProviderSet - should return insurance providers', async ({ request }) => {
  ...
  test('POST /odata/InsuranceProviderSet - should create insurance provider', async ({ request }) => {
    const newProvider = {
...
test.describe('OData Query Options - Patients', () => {
  ...
  test('$filter contains - should filter with contains', async ({ request }) => {
    const response = await request.get(`${ODATA_BASE}/Patients?$filter=contains(first_name,'Test')`);
    expect(response.status()).toBe(200);
  });
...
test.describe('OpenUI5 Frontend - Hospital App', () => {
  ...
  test('should have SAP UI5 framework loaded', async ({ page }) => {
    ...
    const ui5Elements = page.locator('.sapUiBody, [id="sap-ui-bootstrap"], .sapMPage, .sapMList, .sapMTable, .sapMPanel');
  ...
  test('should display master list or navigation', async ({ page }) => {
    ...
    const hasList = await sapList.isVisible().catch(() => false) ||
  ...
  test('should display entity navigation menu', async ({ page }) => {
    ...
    if (await patientNav.isVisible()) {
      ...
      const navPanel = page.locator('.sapMNavContainer, .sapMSplitContainer, nav, [role="navigation"]').first();
  ...
  test('should navigate to patients entity list', async ({ page }) => {
    ...
    const patientLink = page.locator('text=/patient/i, a[href*="Patient"], .sapMLIB:has-text("Patient")').first();
...
test.describe('OData Error Handling', () => {
  ...
  test('should return 404 for non-existent patient ID', async ({ request }) => {
    const response = await request.get(`${ODATA_BASE}/Patients(00000000-0000-0000-0000-000000000000)`);
    expect([404, 400]).toContain(response.status());
  });
