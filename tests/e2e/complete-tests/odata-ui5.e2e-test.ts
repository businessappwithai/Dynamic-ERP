...
test.describe('OData Backend API - Sales Orders', () => {
  ...
  test('GET /odata/SalesOrders with $expand - should include customer details', async ({ request }) => {
...
test.describe('OpenUI5 Frontend', () => {
  ...
  test('should create new item', async ({ page }) => {
    ...
    const createButton = page.locator('button[data-icon="add"], button:has-text("Create"), button:has-text("New")').first();
...
test.describe('OpenUI5 Detail Page', () => {
  ...
  test('should support delete with confirmation', async ({ page }) => {
    ...
    if (await listItem.isVisible()) {
      ...
      const deleteButton = page.locator('button[data-icon="delete"], button:has-text("Delete")').first();
      ...
      if (await deleteButton.isVisible()) {
        ...
        if (await dialog.isVisible({ timeout: 2000 })) {
          // Confirm or cancel
          const hasButtons = await page.locator('button:has-text("Yes"), button:has-text("OK"), button:has-text("No"), button:has-text("Cancel")').count() > 0;
...
test.describe('OData + OpenUI5 Integration', () => {
  test('should sync data between backend and frontend', async ({ page, request }) => {
    ...
    const hasData = (listText ?? '').includes('Sync Test') || (listText ?? '').includes('sync@example');
...
test.describe('OpenUI5 Responsive Design', () => {
  test('should work on mobile viewport', async ({ page }) => {
    ...
    const hasMobileClass = await page.locator('.sapUiRespOnDesktop, .sapUiRespOnMobile, .sapUiRespOnTablet').count() > 0;
