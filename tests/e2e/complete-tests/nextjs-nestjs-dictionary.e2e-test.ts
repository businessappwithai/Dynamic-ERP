...
test.describe('NestJS API - sys_reference (Seeded Data)', () => {
  test('GET /api/sys/references - should return all standard reference types', async ({ request }) => {
  ...
  test('should contain STRING reference type (id=10)', async ({ request }) => {
    const response = await request.get(`${BACKEND_URL}${SYS_REFERENCE_API}/${REFERENCE_TYPES.STRING}`);
  ...
  test('should contain all core reference types', async ({ request }) => {
    ...
    const expectedTypes = [
      REFERENCE_TYPES.STRING, REFERENCE_TYPES.INTEGER, REFERENCE_TYPES.AMOUNT,
      REFERENCE_TYPES.ID, REFERENCE_TYPES.TEXT, REFERENCE_TYPES.DATE,
      REFERENCE_TYPES.DATETIME, REFERENCE_TYPES.LIST, REFERENCE_TYPES.TABLE,
      REFERENCE_TYPES.TABLE_DIRECT, REFERENCE_TYPES.YES_NO, REFERENCE_TYPES.LOCATION,
      REFERENCE_TYPES.LOCATOR, REFERENCE_TYPES.ACCOUNT, REFERENCE_TYPES.URL,
      REFERENCE_TYPES.IMAGE, REFERENCE_TYPES.FILE, REFERENCE_TYPES.COLOR,
      REFERENCE_TYPES.JSON, REFERENCE_TYPES.PASSWORD, REFERENCE_TYPES.EMAIL,
...
test.describe('NestJS API - sys_table CRUD', () => {
  ...
  test('should have sys_table entry for each system table', async ({ request }) => {
    ...
    const expectedSysTables = [
      ...
      'sys_field', 'sys_field_group', 'sys_reference',
    ];
...
test.describe('NestJS API - sys_column CRUD', () => {
  ...
  test('should filter columns by sys_table_id', async ({ request }) => {
    ...
    const response = await request.get(`${BACKEND_URL}${SYS_COLUMN_API}?sys_table_id=${testTableId}`);
  ...
  test('POST /api/sys/columns - should create column with STRING reference', async ({ request }) => {
    if (!testTableId) {
      test.skip(true, 'No test table created');
      return;
    }
  ...
  test('POST /api/sys/columns - should create column with AMOUNT reference', async ({ request }) => {
    if (!testTableId) {
      test.skip(true, 'No test table created');
      return;
    }
  ...
  test('POST /api/sys/columns - should create column with YES_NO reference', async ({ request }) => {
    if (!testTableId) {
      test.skip(true, 'No test table created');
      return;
    }
...
test.describe('NestJS API - sys_window & sys_tab', () => {
  ...
  test('should validate window_type values (M=Maintain, T=Transaction, Q=Query)', async ({ request }) => {
    if (!createdWindowId) {
      test.skip(true, 'No window created');
      return;
    }

    const window = await (await request.get(`${BACKEND_URL}${SYS_WINDOW_API}/${createdWindowId}`)).json();
  ...
  test('GET /api/sys/tabs - should filter tabs by window_id', async ({ request }) => {
    ...
    const response = await request.get(`${BACKEND_URL}${SYS_TAB_API}?sys_window_id=${createdWindowId}`);
...
test.describe('NestJS API - sys_field (Layout & seq_no)', () => {
  ...
  test('PATCH /api/sys/fields/:id - should update seq_no for runtime reordering', async ({ request }) => {
    // Get existing fields for any entity
    const response = await request.get(`${BACKEND_URL}${SYS_FIELD_API}?is_displayed=true&order_by=seq_no`);
    ...
    if (data.data && data.data.length >= 2) {
      ...
      const updateFirst = await request.patch(`${BACKEND_URL}${SYS_FIELD_API}/${firstField.sys_field_id}`, {
        data: { seq_no: originalSecondSeq },
      });
      expect(updateFirst.status()).toBe(200);

      // Update second field with first field's seq_no
      const updateSecond = await request.patch(`${BACKEND_URL}${SYS_FIELD_API}/${secondField.sys_field_id}`, {
  ...
  test('PATCH /api/sys/fields/:id - should toggle is_displayed to hide field', async ({ request }) => {
    const response = await request.get(`${BACKEND_URL}${SYS_FIELD_API}?is_displayed=true`);
    ...
    if (data.data && data.data.length > 0) {
      ...
      // Hide the field
      const hideResponse = await request.patch(`${BACKEND_URL}${SYS_FIELD_API}/${field.sys_field_id}`, {
        data: { is_displayed: false },
      });
      expect(hideResponse.status()).toBe(200);

      // Verify it's hidden
      const verifyResponse = await request.get(`${BACKEND_URL}${SYS_FIELD_API}/${field.sys_field_id}`);
...
test.describe('NestJS API - sys_field_group', () => {
  ...
  test('POST /api/sys/field-groups - should create collapsible field group', async ({ request }) => {
    const newGroup = {
  ...
  test('should support field_group_type values (C=Collapsible, L=Label, T=Tab)', async ({ request }) => {
...
test.describe('Next.js Frontend - Admin Dictionary Interface', () => {
  ...
  test('should display dictionary admin page', async ({ page }) => {
    ...
    await expect(page.locator('h1, h2').filter({ hasText: /Dictionary|Admin|Configuration/i })).toBeVisible();
  });
  ...
  test('should display field layout editor', async ({ page }) => {
    ...
    const fieldEditor = page.locator('[data-testid="field-layout-editor"], .field-editor, [class*="field"]').first();
    ...
    if (await fieldEditor.isVisible()) {
      ...
      const hasFieldProperties = await page.locator('text=/seq|sequence|order|display|visible|group/i').count() > 0;
  ...
  test('should display tables admin page', async ({ page }) => {
    ...
    if (await tableView.isVisible()) {
      ...
      const hasTableColumns = await page.locator('text=/table_name|access_level|is_active/i').count() > 0;
...
test.describe('Next.js Frontend - Dictionary-Driven Dynamic Rendering', () => {
  test('should render entity form fields based on sys_field metadata', async ({ page, request }) => {
    // First get the list of business tables
    const tablesResponse = await request.get(`${BACKEND_URL}${SYS_TABLE_API}`);
    const tablesData = await tablesResponse.json();

    const busTables = tablesData.data?.filter((t: { table_name: string }) => t.table_name.startsWith('bus_'));
  ...
  test('should render entity table columns based on sys_field.is_displayed_grid', async ({ page, request }) => {
    // Get business tables
    const tablesResponse = await request.get(`${BACKEND_URL}${SYS_TABLE_API}`);
    const tablesData = await tablesResponse.json();
  ...
  test('should display entity dashboard listing all bus_ entities from sys_table', async ({ page, request }) => {
    // Get business table count from API
    const tablesResponse = await request.get(`${BACKEND_URL}${SYS_TABLE_API}`);
    const tablesData = await tablesResponse.json();
    const busTableCount = tablesData.data?.filter(
      (t: { table_name: string }) => t.table_name.startsWith('bus_')
    ).length || 0;
...
test.describe('NestJS API - Dictionary Metadata Integrity', () => {
  ...
  test('every sys_column should reference a valid sys_reference type', async ({ request }) => {
    ...
    const validRefIds = refsData.data?.map((r: { sys_reference_id: number }) => r.sys_reference_id) || [];
  ...
  test('identifier columns should be marked with is_identifier=true', async ({ request }) => {
    ...
    if (columnsData.data && columnsData.data.length > 0) {
      ...
      const identifierColumns = columnsData.data.filter((c: { is_identifier: boolean }) => c.is_identifier);
