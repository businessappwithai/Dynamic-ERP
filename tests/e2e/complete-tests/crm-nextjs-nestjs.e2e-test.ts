...
const FRONTEND_URL = process.env.CRM_FRONTEND_URL || process.env.FRONTEND_URL || 'http://localhost:3000';
const BACKEND_URL = process.env.CRM_BACKEND_URL || process.env.BACKEND_URL || 'http://localhost:3001';
...
test.describe('CRM Dashboard Navigation', () => {
  ...
  test('should navigate to companies', async ({ page }) => {
    ...
    const companyLink = page.locator('a[href*="bus_company"], a:has-text("Company"), a:has-text("Companies")').first();
...
test.describe('CRM API - Error Handling', () => {
  test('should return 404 for non-existent company', async ({ request }) => {
    const response = await request.get(`${API_BASE}/companies/00000000-0000-0000-0000-000000000000`);
