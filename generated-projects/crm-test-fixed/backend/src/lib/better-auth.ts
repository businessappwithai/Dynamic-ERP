 ...
 *
 * Modern authentication for CRM Test Fixed
...
export async function initAuth() {
  if (!authInstance) {
    const dbDir = process.env.DATABASE_DIR || path.join(__dirname, '..', '..', 'data', 'crm_test_fixed.db');
    ...
    authInstance = betterAuth({
      ...
      secret: process.env.BETTER_AUTH_SECRET || 'crm_test_fixed-dev-secret',
      emailAndPassword: {
        enabled: true,
      ...
      advanced: {
        cookiePrefix: 'crm_test_fixed_app',
