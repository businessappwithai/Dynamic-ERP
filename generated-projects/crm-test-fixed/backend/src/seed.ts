 ...
 *
 * Generated: 2026-05-16T05:39:22.511Z
 */
...
async function createDialect(): Promise<PostgresDialect | SqliteDialect> {
  if (dbClient === 'better-sqlite3' || dbClient === 'sqlite') {
    ...
    const dbPath = process.env.DATABASE_FILENAME ?? path.join(__dirname, '..', 'data', 'c_r_m _test _fixed.db');
    ...
    return new PostgresDialect({
      pool: new Pool({
        ...
        database: process.env.DATABASE_NAME ?? 'c_r_m _test _fixed',
      }),
    });
  }
}
