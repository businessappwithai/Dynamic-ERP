"""Shared helper functions for ERPClaw Advanced Accounting unit tests."""
import argparse
...
def load_db_query():
    """Load this module's db_query.py and expose all action functions as attributes.

    db_query.py is a unified router that merges ACTIONS dicts from 5 sub-modules.
    We convert each action name (e.g. "add-revenue-contract") into a module attribute
    (e.g. mod.add_revenue_contract) so tests can call them directly.
    """
    db_query_path = os.path.join(SCRIPTS_DIR, "db_query.py")
    spec = importlib.util.spec_from_file_location("db_query_advacct", db_query_path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    # Expose all action functions as module-level attributes
...
def seed_naming_series(conn, company_id):
    series = [
        ("revenue_contract", "RCON-", 0),
        ("lease", "LEAS-", 0),
        ("ic_transaction", "ICT-", 0),
        ("consolidation_group", "CGRP-", 0),
    ]
...
def build_advacct_env(conn) -> dict:
    """Create full advacct test environment with two companies for IC tests."""
    cid1 = seed_company(conn, "Parent Corp", "PC")
    cid2 = seed_company(conn, "Subsidiary Inc", "SI")
    seed_naming_series(conn, cid1)
