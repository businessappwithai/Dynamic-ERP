...
export default class POSTSDetail extends Controller {
  ...
  private _loadRecord(sId: string): void {
    ...
    // Load field metadata
    this._loadFieldMetadata().then((aFields: FieldMetadata[]) => {
    ...
    this._loadTabHierarchy().then((aChildTabs: ChildTab[]) => {
  ...
  private _deleteRecord(): void {
    ...
    oModel.requestOption("DELETE", sPath).then(() => {
      oViewModel.setProperty("/busy", false);
   ...
   */
  private _loadChildData(sBindingKey: string, sTabName: string, sEntitySetName: string, sParentColumnName: string): Promise<any[]> {
    return new Promise((resolve, reject) => {
      const oViewModel = this.getView().getModel("view") as JSONModel;
  ...
  public onTabExpand(oEvent: any): void {
    ...
    this._loadChildData(sBindingKey, sTabName, sEntitySetName, sParentColumnName).then((aData: any[]) => {
      oViewModel.setProperty("/busy", false);
