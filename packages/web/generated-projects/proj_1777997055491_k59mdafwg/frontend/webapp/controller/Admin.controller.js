...
sap.ui.define([
  ...
  return Controller.extend("q-a-test-project.controller.Admin", {
    ...
    _loadSysTables: function() {
      ...
      oModel.bindList("/SysTables", null, [new Sorter("name")], [])
        .requestContexts(0, 500)
        .then(function(aContexts) {
          var aTables = aContexts.map(function(oContext) {
            return Object.assign({}, oContext.getObject());
          });

          // Load field counts for each table
          this._loadFieldCountsForTables(aTables).then(function(aTablesWithCounts) {
            oViewModel.setProperty("/tables", aTablesWithCounts);
            oViewModel.setProperty("/allTables", JSON.parse(JSON.stringify(aTablesWithCounts)));
    ...
    _loadFieldCountsForTables: function(aTables) {
      ...
      return oModel.bindList("/SysFields", null, [], [])
        .requestContexts(0, 2000)
        .then(function(aFieldContexts) {
          var mFieldCounts = {};

          aFieldContexts.forEach(function(oContext) {
          ...
          return aTables.map(function(oTable) {
    ...
    _loadStatistics: function() {
      ...
      // Load column count
      oModel.bindList("/SysColumns", null, [], [])
        .requestContexts(0, 1)
        .then(function(aContexts) {
          // Use the binding's $count if available, or count the total
          oModel.bindList("/SysColumns").requestContexts(0, 5000).then(function(aAllContexts) {
      ...
      // Load field count
      oModel.bindList("/SysFields", null, [], [])
      ...
      // Load window count
      oModel.bindList("/SysWindows", null, [], [])
        .requestContexts(0, 500)
        .then(function(aContexts) {
    ...
    _loadEntities: function() {
      ...
      oModel.getMetaModel().requestObject("/").then(function(oMetadata) {
    ...
    _extractBusinessEntities: function(oMetadata) {
      ...
      return aEntities.sort(function(a, b) {
    ...
    onTableSearch: function(oEvent) {
      ...
      var aFiltered = aTables.filter(function(oTable) {
        return (oTable.name && oTable.name.toLowerCase().indexOf(sQuery) > -1) ||
    ...
    _loadFieldMetadata: function(sEntity) {
      ...
      oModel.bindList("/SysFields", null, [new Sorter(sSortField)], aFilters)
        .requestContexts(0, 200)
        .then(function(aContexts) {
          var aFields = aContexts.map(function(oContext) {
            return Object.assign({}, oContext.getObject());
          });
    ...
    _updateVisibleFields: function() {
      ...
      var aVisible = aFields
        .filter(function(oField) { return oField[sDisplayField]; })
        .sort(function(a, b) { return (a[sSeqField] || 0) - (b[sSeqField] || 0); });
    ...
    _recalculateSeqNumbers: function(aFields) {
      ...
      aFields.forEach(function(oField, nIndex) {
     ...
     * @private
     */
    _getChangedFields: function(aFields, aOriginalFields, sLayoutMode) {
      var aChanged = [];
      var sSeqField = sLayoutMode === "form" ? "seq_no" : "seq_no_grid";
      var sDisplayField = sLayoutMode === "form" ? "is_displayed" : "is_displayed_grid";
      ...
      aFields.forEach(function(oField) {
        ...
        var oOriginal = aOriginalFields.find(function(o) {
          return (o.sys_field_id || o.id) === sFieldId;
        });

        if (oOriginal) {
          if (oField[sSeqField] !== oOriginal[sSeqField] ||
    ...
    _batchUpdateFields: function(aChangedFields, sLayoutMode) {
      ...
      var aPromises = aChangedFields.map(function(oField) {
     ...
     * @private
     */
    _formatEntityName: function(sEntity) {
      return sEntity
        .replace(/^bus_/, "")
        .replace(/_/g, " ")
