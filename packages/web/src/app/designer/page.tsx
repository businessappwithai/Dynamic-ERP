...
export default function DesignerPage() {
  ...
  const [activeTab, setActiveTab] = useState<"editor" | "preview" | "generate">(
    "editor",
  );
  ...
  const validateERD = useCallback(() => {
    ...
    lines.forEach((line, index) => {
      ...
      if (openBraces > closeBraces) {
        // Check if closing brace exists in subsequent lines
    ...
    const hasRelationships = lines.some((line) =>
      relationshipPattern.test(line),
    );
    ...
    <div className="flex h-screen flex-col bg-background">
      ...
      <header className="flex h-14 items-center justify-between border-b bg-card px-4">
        ...
        <div className="flex items-center gap-2">
          {/* AI Assistant button - disabled until CopilotKit dependency is fixed */}
          <button
            onClick={() => alert("AI Assistant coming soon! CopilotKit integration requires ESM package configuration.")}
