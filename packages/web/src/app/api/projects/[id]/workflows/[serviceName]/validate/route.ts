...
/**
 * POST /api/projects/[id]/workflows/[serviceName]/validate
 *
 * Validates hook definitions and flowchart syntax
 */
export async function POST(
  request: NextRequest,
  { params: _params }: { params: { id: string; serviceName: string } }
) {
  try {
    ...
    hooks.forEach((hook: { type?: string; name?: string; entity?: string; code?: string }, index: number) => {
      // Validate hook type
      if (!hook.type || !validHookTypes.includes(hook.type)) {
        validationErrors.push(
          `Hook ${index + 1}: Invalid hook type "${hook.type}"`
        );
      }
      ...
      if (!hook.code || typeof hook.code !== "string") {
        ...
        validationErrors.push(
          `Hook ${index + 1}: Hook code cannot be empty`
        );
      }
      ...
      if (duplicateCount > 1) {
        validationErrors.push(
          `Hook ${index + 1}: Duplicate hook "${hook.type}:${hook.name}"`
        );
      }
    });
    ...
    if (!flowchartCode || typeof flowchartCode !== "string") {
      ...
      validationErrors.push(
        "Flowchart code must start with 'flowchart' keyword"
      );
    }
