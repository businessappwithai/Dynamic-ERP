...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate each node
    jdm.nodes.forEach((node: { id?: string; type?: string; content?: Record<string, unknown> }, index: number) => {
      if (!node.id) {
      ...
      if (node.type === "decisionTable") {
        if (!node.content) {
          ...
          if (!node.content.rules || !Array.isArray(node.content.rules)) {
            ...
            // Validate each rule
            node.content.rules.forEach((rule: { condition?: string; output?: unknown }, ruleIndex: number) => {
              if (!rule.condition) {
