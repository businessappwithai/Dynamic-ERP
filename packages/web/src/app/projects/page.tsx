...
export default function ProjectsPage() {
  ...
  const handleNewProject = async (data: {
    ...
    try {
      // Find an available port starting from 4001
      ...
      router.push(`/projects/${newProject.id}/init`);
    } catch (error) {
    ...
    <div className="min-h-screen bg-background">
        ...
        {isLoading && projects.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProjects.map((project) => {
              const colorClass =
                colorMap[project.iconColor] || colorMap["#3b82f6"];
              const isRunning = project.deploymentStatus === "running";

              // Status badge styling
              const getStatusBadge = () => {
                if (isRunning) {
                  return { text: "Active", className: "bg-emerald-500/20 text-emerald-400 border-emerald-500/30" };
                }
                if (project.generatedPath) {
                  return { text: "Complete", className: "bg-blue-500/20 text-blue-400 border-blue-500/30" };
                }
                return { text: "Draft", className: "bg-amber-500/20 text-amber-400 border-amber-500/30" };
              };
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    ...
                    if (project.generatedPath) {
                      router.push(`/projects/${project.id}/enhance`);
                    } else {
                      router.push(`/projects/${project.id}/init`);
                    }
                  }}
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold uppercase tracking-wide border ${statusBadge.className}`}>
                      {statusBadge.text}
                    </span>
                  </div>
                  ...
                  <div className="flex gap-2">
                    {/* Show Edit for new projects, Enhance for generated projects */}
                  ...
                  {showMenu === project.id && (
                    <>
                      ...
                      />
                      <div
                        className="absolute right-0 bottom-16 w-48 bg-card border border-border rounded-lg shadow-2xl z-50 overflow-hidden py-1">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-card border border-border rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              This will mark the project for deletion. It will be removed from
      ...
      {showLogsViewer && (
        <LogsViewer
          ...
          projectName={projects.find(p => p.id === useProjectStore.getState().currentProject?.id)?.name}
        />
      )}
