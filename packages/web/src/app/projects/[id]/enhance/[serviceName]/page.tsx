...
export default function ServiceWorkflowPage() {
  ...
  // Extract entities from project ERD code
  ...
  // Load workflows from API and project
  useEffect(() => {
    const loadWorkflow = async () => {
      if (project) {
        setCurrentStep("enhance");

        // Try to load hook workflow from API first
        try {
  ...
  // Mark as dirty when hooks or flowchart code change
  useEffect(() => {
  ...
  const updateFlowchartWithHooks = (hooks: HookDefinition[]) => {
    ...
    // Convert hooks to ParsedHookDefinition format
    ...
    // Generate flowchart using the utility function
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {!showGeneratedCode ? (
        <div className="flex-1 flex overflow-hidden">
          {activeTab === "hooks" ? (
            <>
          {/* Left Panel - Available Hooks & Active Hooks */}
          ...
          {/* Right Panel - Flowchart Code & Preview */}
          <div className="flex-1 flex flex-col bg-muted">
            {/* Flowchart Code Editor - Top Section */}
            ...
            {/* Flowchart Preview - Collapsible Bottom Panel */}
            {showFlowchartPreview && (
