...
import { Info, Settings, User, Loader2, ArrowLeft, Check } from "lucide-react";
  ...
  useEffect(() => {
    if (currentProject) {
      setFormData({
        ...
        databaseUrl: currentProject.databaseUrl || `postgresql://user:password@localhost:5432/${projectId}`,
      });
      setCurrentProject(projectId);
    }
  }, [currentProject, projectId, setCurrentProject]);

  // Determine if this is a new project (just created, never generated)
  const isNewProject = currentProject ? !currentProject.generatedPath && currentProject.deploymentStatus !== "completed" : false;
                  ...
                  <div className="space-y-2">
                    <label className={`flex items-center p-4 border-2 rounded-xl cursor-pointer transition-colors ${
                      ...
                      <div className="flex-1">
                        <p className="font-semibold text-foreground">Full Stack: Next.js + NestJS</p>
                  ...
                  {isNewProject ? (
                    <>
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, databaseUrl: e.target.value })
                        }
                      ...
                      />
                      <p className="text-xs text-muted-foreground mt-1">
                        Using database: <span className="font-mono">{projectId}</span> (default: SQLite)
                      </p>
                    </>
                  ) : (
                    <>
