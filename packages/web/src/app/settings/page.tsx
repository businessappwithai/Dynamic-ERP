...
export default function SettingsPage() {
    ...
    const handleNotificationChange = async (key: keyof typeof notifications, value: boolean) => {
        // For push notifications, check permissions first
    ...
    const loadCacheInfo = async () => {
        ...
        try {
            ...
            // Get last cleared from localStorage
    ...
    return (
        <div className="min-h-screen bg-background">
                            ...
                            {notificationPermission === "denied" && (
                                <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                                    <p className="text-xs text-amber-600 dark:text-amber-400">
                                        Push notifications are blocked. To enable them, go to your browser settings and allow notifications for this site.
                                    </p>
                                </div>
                            )}
                        ...
                        <div className="space-y-3">
                            <button
                                onClick={() => setShowConfirmDialog({
                                    ...
                                    description: "This will clear all cached data including images, API responses, and temporary files. Your projects and settings will be preserved.",
                                })}
                            ...
                            <button
                                onClick={() => setShowConfirmDialog({
                                    ...
                                    description: "This will remove all temporary files, session data, and IndexedDB entries. This action helps improve application performance.",
                                })}
                            ...
                            <button
                                onClick={() => setShowConfirmDialog({
                                    ...
                                    description: "Download a copy of all your data including settings, preferences, and local storage. This is useful for backups or migrating to another device.",
                                })}
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    className={`relative w-11 h-5.5 rounded-full transition-colors ${privacySettings.analytics ? "bg-primary" : "bg-muted"
                                        }`}
                                >
                                    <div
                                        className={`absolute top-0.5 w-4.5 h-4.5 bg-white rounded-full transition-transform ${privacySettings.analytics ? "translate-x-5.5" : "translate-x-0.5"
                                            }`}
                                    />
                                </button>
                            </div>
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    className={`relative w-11 h-5.5 rounded-full transition-colors ${privacySettings.crashReports ? "bg-primary" : "bg-muted"
                                        }`}
                                >
                                    <div
                                        className={`absolute top-0.5 w-4.5 h-4.5 bg-white rounded-full transition-transform ${privacySettings.crashReports ? "translate-x-5.5" : "translate-x-0.5"
                                            }`}
                                    />
                                </button>
                            </div>
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    onClick={() => handlePrivacyChange("personalization", !privacySettings.personalization)}
                                    className={`relative w-11 h-5.5 rounded-full transition-colors ${privacySettings.personalization ? "bg-primary" : "bg-muted"
                                        }`}
                                >
                                    <div
                                        className={`absolute top-0.5 w-4.5 h-4.5 bg-white rounded-full transition-transform ${privacySettings.personalization ? "translate-x-5.5" : "translate-x-0.5"
                                            }`}
                                    />
                                </button>
                            </div>
                            ...
                            {showPrivacyDetails && (
                                <div className="mt-4 p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl space-y-2">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        ...
                                        personal information without your consent. You can export or delete your data
                                        at any time using the options above.
                                    </p>
                                </div>
                            )}
                            ...
                            <div className="pt-4 border-t border-border">
                                <button
                                    onClick={() => setShowConfirmDialog({
                                        ...
                                        description: "⚠️ This will permanently delete all your data including projects, settings, preferences, and cached information. This action cannot be undone.",
                                        warning: "This is a destructive action that will remove all your data from this device.",
                                    })}
            ...
            {/* Action Status Toast */}
            {actionStatus.type && (
                <div className={`fixed bottom-24 left-4 right-4 max-w-screen-md mx-auto p-4 rounded-xl shadow-2xl z-50 ${
