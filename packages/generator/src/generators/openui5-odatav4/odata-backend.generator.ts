...
import { tableNameToControllerName, tableNameToEntitySetName } from '@erdwithai/core/utils/table-naming';
...
export class ODataBackendGenerator extends BaseGenerator {
  ...
  private async generateCoreFiles(outputDir: string, context: any): Promise<void> {
    // Database connection
    const dbConnectionContent = await this.renderTemplate('src/database/connection.ts.hbs', context);
    ...
    const dynamicSchemaContent = await this.renderTemplate('src/utils/dynamic-schema.ts.hbs', context);
    ...
    const odataFormatterContent = await this.renderTemplate('src/utils/odata-formatter.ts.hbs', context);
  ...
  private async generateControllerGenerators(outputDir: string, context: any): Promise<void> {
    ...
    const generateControllersContent = await this.renderTemplate('src/generate-controllers.ts.hbs', context);
    await fs.writeFile(path.join(outputDir, 'src/generate-controllers.ts'), generateControllersContent);

    // Generate the server class generation script (creates server.ts with all decorators)
    const createServerClassContent = await this.renderTemplate('src/create-server-class.ts.hbs', context);
    await fs.writeFile(path.join(outputDir, 'src/create-server-class.ts'), createServerClassContent);
  }
