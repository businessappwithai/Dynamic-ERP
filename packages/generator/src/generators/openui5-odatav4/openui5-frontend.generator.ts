...
import { entityToBusEntity, attributeToBusAttribute } from '@erdwithai/core/types';
...
export class OpenUI5FrontendGenerator extends BaseGenerator {
  ...
  private async generateWebappFiles(outputDir: string, context: any): Promise<void> {
    ...
    try {
      const messageHelperContent = await this.renderTemplate('webapp/utils/MessageHelper.ts.hbs', context);
      await fs.writeFile(path.join(outputDir, 'webapp/utils/MessageHelper.ts'), messageHelperContent);
    } catch (e) {
  ...
  private async generateViews(outputDir: string, context: any): Promise<void> {
    ...
    for (const view of baseViews) {
      try {
        ...
        await fs.writeFile(path.join(outputDir, `webapp/view/${view.replace('.hbs', '')}`), viewContent);
      } catch (e) {
  ...
  private async generateControllers(outputDir: string, context: any): Promise<void> {
    ...
    for (const controller of baseControllers) {
      ...
      try {
        const controllerContent = await this.renderTemplate(`webapp/controller/${controller}.ts.hbs`, context);
        await fs.writeFile(path.join(outputDir, `webapp/controller/${controller}.ts`), controllerContent);
      } catch (_e) {
        // Fallback to JavaScript
        try {
          const controllerContent = await this.renderTemplate(`webapp/controller/${controller}.js.hbs`, context);
          await fs.writeFile(path.join(outputDir, `webapp/controller/${controller}.js`), controllerContent);
        } catch (_e2) {
  ...
  private async generateTestFiles(outputDir: string, context: any): Promise<void> {
    try {
      ...
      const opaTestsHtml = await this.renderTemplate('webapp/test/integration/opaTests.qunit.html.hbs', context);
      await fs.writeFile(path.join(outputDir, 'webapp/test/integration/opaTests.qunit.html'), opaTestsHtml);

      // OPA5 test JS runner
      const opaTestsJs = await this.renderTemplate('webapp/test/integration/opaTests.qunit.js.hbs', context);
      await fs.writeFile(path.join(outputDir, 'webapp/test/integration/opaTests.qunit.js'), opaTestsJs);

      // AllJourneys
      const allJourneys = await this.renderTemplate('webapp/test/integration/AllJourneys.js.hbs', context);
      await fs.writeFile(path.join(outputDir, 'webapp/test/integration/AllJourneys.js'), allJourneys);

      // Navigation Journey
      const navigationJourney = await this.renderTemplate('webapp/test/integration/NavigationJourney.js.hbs', context);
      await fs.writeFile(path.join(outputDir, 'webapp/test/integration/NavigationJourney.js'), navigationJourney);

      // Entity CRUD Journey
      const crudJourney = await this.renderTemplate('webapp/test/integration/EntityCRUDJourney.js.hbs', context);
      await fs.writeFile(path.join(outputDir, 'webapp/test/integration/EntityCRUDJourney.js'), crudJourney);

      // Page objects
      const appPage = await this.renderTemplate('webapp/test/integration/pages/App.js.hbs', context);
      await fs.writeFile(path.join(outputDir, 'webapp/test/integration/pages/App.js'), appPage);

      const listPage = await this.renderTemplate('webapp/test/integration/pages/List.js.hbs', context);
      await fs.writeFile(path.join(outputDir, 'webapp/test/integration/pages/List.js'), listPage);

      const detailPage = await this.renderTemplate('webapp/test/integration/pages/Detail.js.hbs', context);
      await fs.writeFile(path.join(outputDir, 'webapp/test/integration/pages/Detail.js'), detailPage);
  ...
  private async generatePerEntityFiles(outputDir: string, context: any): Promise<void> {
    for (const entity of context.entities) {
      ...
      for (const { base, output } of controllerBases) {
        // Try TypeScript first
        try {
          const content = await this.renderTemplate(`webapp/controller/entity/${base}.ts.hbs`, entityContext);
          await fs.writeFile(path.join(outputDir, `webapp/controller/entity/${output}.ts`), content);
        } catch (_e) {
          // Fallback to JavaScript
          try {
            const content = await this.renderTemplate(`webapp/controller/entity/${base}.js.hbs`, entityContext);
            await fs.writeFile(path.join(outputDir, `webapp/controller/entity/${output}.js`), content);
          } catch (_e2) {
      ...
      const viewTemplates = [
        { template: 'webapp/view/entity/EntityList.view.xml.hbs', output: `${entityPascal}List.view.xml` },
        { template: 'webapp/view/entity/EntityDetail.view.xml.hbs', output: `${entityPascal}Detail.view.xml` },
        { template: 'webapp/view/entity/EntityCreate.view.xml.hbs', output: `${entityPascal}Create.view.xml` },
      ];
