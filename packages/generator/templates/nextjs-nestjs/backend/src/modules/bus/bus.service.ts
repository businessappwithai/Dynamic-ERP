...
export class BusService {
  ...
  async getAllFormFields(entity: string): Promise<FieldMetadata[]> {
    ...
    const columns = await this.knex('sys_column')
      ...
      .leftJoin('sys_field_group as sys_field_group', 'sys_field.sys_field_group_id', 'sys_field_group.sys_field_group_id')
      .where('sys_column.sys_table_id', table.sys_table_id)
      .where('sys_column.is_active', true)
  ...
  async getFieldGroups(entity: string): Promise<FieldGroup[]> {
    ...
    return groups.map((g: any) => ({
      ...
      is_collapsed_by_default: g.is_collapsed_by_default === 1 || g.is_collapsed_by_default === true,
    }));
  }
   ...
   */
  async createFieldGroup(entity: string, groupData: {
    name: string;
    description?: string;
    seq_no?: number;
    columns?: number;
   ...
   */
  async updateFieldGroup(entity: string, groupId: string, updates: Partial<FieldGroup>): Promise<FieldGroup> {
    ...
    await this.knex('sys_field_group')
   ...
   */
  async updateFieldStyle(entity: string, fieldId: string, style: {
    color?: string;
    col_span?: number;
    row_span?: number;
    font_weight?: string;
    ...
    for (const column of metadata.columns) {
      ...
      if (value && column.field_length && typeof value === 'string') {
        if (value.length > column.field_length) {
          errors.push(
            `Field '${column.name}' exceeds maximum length of ${column.field_length}`,
          );
        }
      }
